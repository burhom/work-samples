 ...
 * Define Constants
 */
define('roots_shortname', 'roots'); // used to prefix the individual setting field id see roots_options_page_fields()
define('roots_base', 'roots-options'); // the settings page slug
 ...
 */
add_action( 'admin_menu', 'roots_options_add_menu' );


/*
 * Group scripts (js & css)
 */
function roots_options_scripts(){
  wp_enqueue_style('roots_theme_settings_css', get_template_directory_uri() . '/lib/css/theme-options.css');
  wp_enqueue_script( 'roots_theme_settings_js', get_template_directory_uri() . '/lib/js/theme-options.js', array('jquery'));
}
...
function roots_options_add_menu(){
  ...
  $roots_options_page = add_menu_page(__('Site Settings'), __('Site Settings','roots_textdomain'), 'level_7', roots_base, 'roots_options_page_fn', '',54.5);
  // contextual help
 ...
 * @return array
 */
 function roots_get_settings() {

  $output = array();

  // put together the output array
  $output['roots_option_name']    = 'roots_options'; // the option name as used in the get_option() call.
  $output['roots_page_title']     = __( 'Site Settings','roots_textdomain'); // the settings page title
  $output['roots_page_sections']    = roots_options_page_sections(); // the setting section
  $output['roots_page_fields']    = roots_options_page_fields(); // the setting fields
  $output['roots_contextual_help']  = roots_options_page_contextual_help(); // the contextual help
 ...
 * Contextual Help
 */
function roots_options_page_contextual_help() {
  $title = get_bloginfo('name').' - Settings';

  $text   = "<h3>" . __($title,'roots_textdomain') . "</h3>";
  $text   .= "<p>" . __('If you need help please contact Ibrahim Muhammad directly.','roots_textdomain') . "</p>";
 ...
 */
function roots_create_settings_field( $args = array() ) {
  ...
  $defaults = array(
    ...
    'section' => 'main_section',            // the section this setting belongs to — must match the array key of a section in roots_options_page_sections()
  ...
  // additional arguments for use in form field output in the function roots_form_field_fn!
  $field_args = array(
    'type'      => $type,
    'id'        => $id,
    'desc'      => $desc,
  ...
  add_settings_field( $id, $title, 'roots_form_field_fn', __FILE__, $section, $field_args );
...
function roots_register_settings(){
  ...
  $settings_output  = roots_get_settings();
  $roots_option_name = $settings_output['roots_option_name'];

  //setting
  // register_setting( $option_group, $option_name, $sanitize_callback );
  register_setting($roots_option_name, $roots_option_name, 'roots_validate_options' );

  //sections
  // add_settings_section( $id, $title, $callback, $page );
  if(!empty($settings_output['roots_page_sections'])){
    // call the "add_settings_section" for each!
    foreach ( $settings_output['roots_page_sections'] as $id => $title ) {
      add_settings_section( $id, $title, 'roots_section_fn', __FILE__);
    }
  }

  //fields
  if(!empty($settings_output['roots_page_fields'])){
    // call the "add_settings_field" for each!
    foreach ($settings_output['roots_page_fields'] as $option) {
      roots_create_settings_field($option);
    }
  }
}
 ...
 * @return array
 */
function roots_validate_options($input) {
  ...
  $options = $settings_output['roots_page_fields'];
        ...
        case 'numeric':
              ...
              if(is_numeric($input[$option['id']]) == FALSE) {
                add_settings_error(
                  ...
                  roots_shortname . '_txt_numeric_error', // error ID
                  __('Expecting a Numeric value! Please fix.','roots_textdomain'), // error message
            ...
            //for multi-numeric values (separated by a comma)
              ...
              if($input[$option['id']] !=''){
                ...
                $valid_input[$option['id']] = (preg_match('/^-?\d+(?:,\s?-?\d+)*$/', $input[$option['id']]) == 1) ? $input[$option['id']] : __('Expecting comma separated numeric values','roots_textdomain');
              }else{
              ...
              if($input[$option['id']] !='' && preg_match('/^-?\d+(?:,\s?-?\d+)*$/', $input[$option['id']]) != 1) {
                add_settings_error(
                  $option['id'], // setting title
                  roots_shortname . '_txt_multinumeric_error', // error ID
                  __('Expecting comma separated numeric values! Please fix.','roots_textdomain'), // error message
                  'error' // type of message
                  );
              }
              break;
            ...
            //for email
              ...
              $input[$option['id']]     = trim($input[$option['id']]); // trim whitespace
              if($input[$option['id']] != ''){
                $valid_input[$option['id']] = (is_email($input[$option['id']])!== FALSE) ? $input[$option['id']] : __('Invalid email! Please re-enter!','roots_textdomain');
              }elseif($input[$option['id']] == ''){
                $valid_input[$option['id']] = __('This setting field cannot be empty! Please enter a valid email address.','roots_textdomain');
              }
              ...
              if(is_email($input[$option['id']])== FALSE || $input[$option['id']] == '') {
                add_settings_error(
                  $option['id'], // setting title
                  roots_shortname . '_txt_email_error', // error ID
                  __('Please enter a valid email address.','roots_textdomain'), // error message
                  'error' // type of message
                  );
              }
              break;
 ...
 */
function roots_show_msg($message, $msgclass = 'info') {
 ...
 *
 * @return calls roots_show_msg()
 */
function roots_admin_msgs() {

  // check for our settings page - need this in conditional further down
  $roots_settings_pg = strpos($_GET['page'], roots_base);
  ...
  if(current_user_can ('level_7') && $roots_settings_pg !== FALSE && !empty($set_errors)){

    // have our settings succesfully been updated?
    if($set_errors[0]['code'] == 'settings_updated' && isset($_GET['settings-updated'])){
      roots_show_msg("<p>" . $set_errors[0]['message'] . "</p>", 'updated');
    ...
    // have errors been found?
      ...
      foreach($set_errors as $set_error){
        ...
        roots_show_msg("<p class='setting-error-message' title='" . $set_error['setting'] . "'>" . $set_error['message'] . "</p>", 'error');
      }
    }
  }
}
...
// admin messages hook!
 add_action('admin_notices', 'roots_admin_msgs');
 ...
 * @return array
 */
function roots_options_page_sections() {

  $sections = array();

  $sections['core_section']   = __('Core Settings', 'roots_textdomain');
 ...
 * @return array
 */
function roots_options_page_fields() {

  // google analytics
  $options[] = array(
    "section" => "core_section",
    "id"      => roots_shortname . "_ga",
    "title"   => __( 'Google Analytics ID', 'roots_textdomain' ),
    "desc"    => __( 'Enter your UA-XXXXX-X ID', 'roots_textdomain' ),
    "type"    => "text",
    "std"     => __('','roots_textdomain')
    );
  ...
  if ( current_user_can('edit_themes') == true) {
    ...
    $options[] = array(
      "section" => "core_section",
      "id"      => roots_shortname . "_show_posts",
      "title"   => __( 'Show Posts?', 'roots_textdomain' ),
      "desc"    => __( 'Enable news/press functionality', 'roots_textdomain' ),
      "type"    => "select",
      "std"     => __('yes','roots_textdomain'),
 ...
 */
function  roots_section_fn($desc) {
 ...
 * @return echoes output
 */
function roots_options_page_fn() {
// get the settings sections array
  $settings_output = roots_get_settings();
  ?>
  <div class="wrap" id="roots-options-container">
    <h2><?php echo $settings_output['roots_page_title']; ?></h2>

    <form action="options.php" method="post">
      <?php
      // http://codex.wordpress.org/Function_Reference/settings_fields
      settings_fields($settings_output['roots_option_name']);
      ...
      <p class="submit">
        <input name="Submit" type="submit" class="button-primary" value="<?php esc_attr_e('Save Changes','roots_textdomain'); ?>" />
      </p>
 ...
 * @return echoes output
 */
function roots_form_field_fn($args = array()) {
  ...
  $roots_option_name = $settings_output['roots_option_name'];
  $options      = get_option($roots_option_name);
  ...
  switch ( $type ) {
    ...
    echo "<input class='regular-text$field_class' type='text' id='$id' name='" . $roots_option_name . "[$id]' value='$options[$id]' />";
    ...
    foreach($choices as $item) {
      ...
      $item[0] = esc_html__($item[0], 'roots_textdomain');
      if (!empty($options[$id])) {
      ...
      echo "<span>$item[0]:</span> <input class='$field_class' type='text' id='$id|$item[1]' name='" . $roots_option_name . "[$id|$item[1]]' value='$value' /><br/>";
    }
    ...
    echo "<textarea class='textarea$field_class' type='text' id='$id' name='" . $roots_option_name . "[$id]' rows='5' cols='50'>$options[$id]</textarea>";
    echo ($desc != '') ? "<br /><span class='description'>$desc</span>" : "";
    break;

    case 'select':
    echo "<select id='$id' class='select$field_class' name='" . $roots_option_name . "[$id]'>";
    foreach($choices as $item) {
      $value  = esc_attr($item, 'roots_textdomain');
      $item   = esc_html($item, 'roots_textdomain');
    ...
    foreach($choices as $item) {
      ...
      $item = explode("|",$item);
      $item[0] = esc_html($item[0], 'roots_textdomain');
    ...
    case 'checkbox':
    echo "<input class='checkbox$field_class' type='checkbox' id='$id' name='" . $roots_option_name . "[$id]' value='1' " . checked( $options[$id], 1, false ) . " />";
    echo ($desc != '') ? "<br /><span class='description'>$desc</span>" : "";
    break;
    ...
    foreach($choices as $item) {
      ...
      echo "<input class='checkbox$field_class' type='checkbox' id='$id|$item[1]' name='" . $roots_option_name . "[$id|$item[1]]' value='1' $checked /> $item[0] <br/>";
    }
    echo ($desc != '') ? "<br /><span class='description'>$desc</span>" : "";
    break;
  }
